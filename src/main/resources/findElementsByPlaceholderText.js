...
// minify using `uglifyjs findElementsByPlaceholderText.js -c -m reserved=['findAllByPlaceholderText'] -o findElementsByPlaceholderText.min.js`
if (!Array.from) {
    Array.from = (function () {
        var symbolIterator;
        try {
...
function findAllByPlaceholderText(container, ariaLabel) {
  checkContainerType(container);
  const matcher = matches;
  const matchWith = getMatchWith(ariaLabel);
  return Array.from(container.querySelectorAll('*')).filter(function (node) {
    return matcher(node.getAttribute('placeholder'), matchWith);
  });
}
