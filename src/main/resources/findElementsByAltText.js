...
// minify using `uglifyjs findElementsByAltText.js -c -m reserved=['findAllByAltText'] -o findElementsByAltText.min.js`
if (!Array.from) {
    Array.from = (function () {
        var symbolIterator;
        try {
...
function findAllByAltText(container, alt) {
  checkContainerType(container);
  const matcher = matches;
  const matchWith = getMatchWith(alt);
  return Array.from(container.querySelectorAll('img,input,area')).filter(
    function (node) {
      return matcher(node.getAttribute('alt'), matchWith);
    }
  );
}
