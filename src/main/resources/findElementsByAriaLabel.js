...
// minify using `uglifyjs findElementsByAriaLabel.js -c -m reserved=['findAllByAriaLabel'] -o findElementsByAriaLabel.min.js`
if (!Array.from) {
    Array.from = (function () {
        var symbolIterator;
        try {
...
function findAllByAriaLabel(container, ariaLabel) {
  ...
  return Array.from(container.querySelectorAll('*')).filter(function (node) {
    return matcher(node.getAttribute('aria-label'), matchWith);
  });
}
