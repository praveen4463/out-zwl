...
// minify using `uglifyjs findElementsByTitle.js -c -m reserved=['findAllByTitle'] -o findElementsByTitle.min.js`
const TEXT_NODE = 3;
...
function findAllByTitle(container, title) {
  checkContainerType(container);
  const matcher = matches;
  const matchWith = getMatchWith(title);
  return Array.from(container.querySelectorAll('[title], svg > title')).filter(
    function (node) {
      return (
        matcher(node.getAttribute('title'), matchWith) ||
        (isSvgTitle(node) && matcher(getNodeText(node), matchWith))
      );
    }
  );
}
